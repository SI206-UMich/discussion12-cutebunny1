...
def create_employee_table(cur, conn):
    cur.execute("CREATE TABLE IF NOT EXISTS Employees (employee_id INTEGER PRIMARY KEY, first_name TEXT, last_name TEXT, job_id INT, salary INT)") # specify the primary key
    conn.commit()
...
def add_employee(filename, cur, conn):
    ...
    for item in employee_data:
        ...
        cur.execute('INSERT OR IGNORE INTO Employees (employee_id, first_name, last_name, hire_date, job_id, salary) VALUES (?,?,?,?,?,?)', (employee_id, first_name, last_name, hire_date, job_id, salary)) # avoid adding the same data several times
    conn.commit()
...
def problematic_salary(cur, conn):
    ...
    invalid_names = []
    for item in invalid:
        invalid_names.append(item[0] + ' ' + item[1])
